...
abstract class Model 
{
	 ...
	 * @return mixed La valeur de retour de la méthode execute()
	 */
	public function deleteFavoris($idUser, $idPlace)
	{
		if (!is_numeric($idUser) && !is_numeric($idFav)){
			return false;
		}

		$sql = 'DELETE FROM `favorites` WHERE `users_us_id`= :idUser AND `places_pl_id`= :idPlace';


		$sth = $this->dbh->prepare($sql);
		$sth->bindValue(':idUser', $idUser);
		$sth->bindValue(':idPlace', $idPlace);
		return $sth->execute();
	}
	...
	public function insert(array $data, $stripTags = true)
	{
		...
		$colNames = array_keys($data); //clès des données en tableau
		$colNamesEscapes = $this->escapeKeys($colNames); //escapeKeys est une fonction qui sert à ajouter `` a une chaine de caractere
		$colNamesString = implode(', ', $colNamesEscapes); //implode : rassemble les éléments d'un tableau en une chaîne
		...
		// Supprime les caractères superflus en fin de requète
		$sql = substr($sql, 0, -2); //0 pour aller a la fin, efface les 2 derniers elements
		$sql .= ')';

		$sth = $this->dbh->prepare($sql);
		foreach($data as $key => $value){
			$value = ($stripTags) ? strip_tags($value) : $value; //if sur une seule ligne : ternaire  condition ? oui : sinon ;
			//strip_tags pour supp les html & php par securité
			$sth->bindValue(':'.$key, $value); 
		}
...
///////////////function que je crée
		/**
	 * Teste si un endroit/place est présent en base de données
	 * @param string $username Le pseudo à tester
	 * @return boolean true si présent en base de données, false sinon
	 */
	public function placeExists($place)
	{

	    $app = getApp();

	    $sql = 'SELECT ' . $app->getConfig('security_place_property') . ' FROM ' . $this->table .
	           ' WHERE ' . $app->getConfig('security_place_property') . ' = :place LIMIT 1';

	    $dbh = ConnectionModel::getDbh();
	    $sth = $dbh->prepare($sql);
	    $sth->bindValue(':place', $place);
	    if($sth->execute()){
	        $foundUser = $sth->fetch();
	        if($foundUser){
	            return true;
	...
	/**
	 * Teste si une recipe est présent en base de données
	 * @param string $username Le pseudo à tester
	 * @return boolean true si présent en base de données, false sinon
	 */
	public function recipeExists($recipe)
	{
	    ...
	    $sql = 'SELECT ' . $app->getConfig('security_recipe_property') . ' FROM ' . $this->table .
	           ' WHERE ' . $app->getConfig('security_recipe_property') . ' = :recipe LIMIT 1';

	    $dbh = ConnectionModel::getDbh();
	    $sth = $dbh->prepare($sql);
	    $sth->bindValue(':recipe', $recipe);
	    if($sth->execute()){
	        $foundUser = $sth->fetch();
	        if($foundUser){
	            return true;
	...
	/**
	 * Teste si un ingredient est présent en base de données
	 * @param string $username Le pseudo à tester
	 * @return boolean true si présent en base de données, false sinon
	 */
	public function ingredientExists($ingredient)
	{

	    $app = getApp();

	    $sql = 'SELECT ' . $app->getConfig('security_ingredient_property') . ' FROM ' . $this->table .
	           ' WHERE ' . $app->getConfig('security_ingredient_property') . ' = :ingredient LIMIT 1';

	    $dbh = ConnectionModel::getDbh();
	    $sth = $dbh->prepare($sql);
	    $sth->bindValue(':ingredient', $ingredient);
	    if($sth->execute()){
	        $foundUser = $sth->fetch();
	        if($foundUser){
	            return true;
